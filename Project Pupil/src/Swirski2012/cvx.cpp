...
float pupiltracker::cvx::histKmeans(const cv::Mat_<float>& hist, int bin_min, int bin_max, int K, \
	float init_centres[], cv::Mat_<uchar>& labels, cv::TermCriteria termCriteria)
{
    ...
    while (true)
    {
		...
		//计算每个点(即bins)离两个center的距离，按距离短的选择label
		//这部分其实可以适合这种hist做优化，两类的中点即是分割label的位置，而没必要每个点都计算距离
