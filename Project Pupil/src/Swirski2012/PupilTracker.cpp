...
//直接在code中实现符号替换，所以SECTION{}中的{}是else的部分；if()内部是bool
...
public:
    HaarSurroundFeature(int r1, int r2) : r_inner(r1), r_outer(r2)
    {
        ...
        val_outer = -val_inner*count_inner/count_outer; //val_inner*count_inner = 1
		//这个给的是权重，实际上就是计算平均值的差值
    }
    ...
    )
{
    ...
    SECTION("Haar responses", log)
    {
        ...
        {   
			...
			//我添加的代码，主要是考虑到设置的params.Radius_Max过大，超出范围会报错
			if (((mEye.rows - r - r - 1) / ystep + 1) / 8 <= 0)
				continue;
                ...
                [&] (tbb::blocked_range<int> range, const std::pair<double,cv::Point2f>& minValIn) -> std::pair<double,cv::Point2f>
            {
                ...
                for (int i = range.begin(), y = r + range.begin()*ystep; i < range.end(); i++, y += ystep)
                {
                    ...
                    //            |         |  p10'                     'p11
                    //            ?        ?
					//原理：利用指针访问积分图的四个元素，先访问行，再访问列。
					//个人评价：显得十分啰嗦，直接访问不是更方便吗？
    ...
    // ---------------------------
	//pHaarPupil是rect的中心，该函数即计算完整Rect
	...
	//两个centers和一个threshold其实是等效的，所以这个方法和基本全局阈值处理方法一致
    float threshold;
    SECTION("KMeans", log)
    {
        ...
        for (int i = 0; i < 2; i++) //两次循环，两种kmeans初始点
        {
            cv::Mat_<uchar> labels;
            float centres[2] = {candidate0[i], candidate1[i]};
            float dist = cvx::histKmeans(hist, 0, 256, 2, centres, labels, \
    ...
    SECTION("Find best region", log)
    {
        ...
        elPupilThresh = cvx::fitEllipse(momentsPupilThresh); //该部分有替代的功能
    ...
    out.bbPupilThresh = bbPupilThresh; //计算bounding box
    out.elPupilThresh = elPupilThresh; //计算min area rect
    ...
    SECTION("Pupil preprocessing", log)
    {
		...
		//padding主要是为了canny处理的需要，并不是真的扩展了roiPupil
