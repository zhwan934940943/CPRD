...
void LPW_init_iris()
{
	allDatasets_dir_ = "C:/Users/w9349/OneDrive - Platinum/mycodelib/pupil datasets/";

	dataset_name_ = "LPW";
	dataset_dir_ = allDatasets_dir_ + dataset_name_ + "/";
	//caselist保存每个case的文件前缀名
	readStringList_txt(dataset_dir_ + "caselist.txt", caselist_);

	results_dir_ = allDatasets_dir_ + "error/";


	//init rect的路径与导入
	//init_rect_pupil.txt是瞳孔init rect
	//init_rect2.txt是虹膜init rect
	ifstream fin(dataset_dir_ + "init_rect2.txt");
	double x, y, width, height;
	while (fin >> x)
	{
		fin >> y >> width >> height;
		init_rectlist_.push_back(Rect(x, y, width, height / 2));
	}
}
